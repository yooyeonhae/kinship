...
**원격 대전의 판 동기화는 Broadcast로 "바뀌었다"만 알리고 내용은 DB에서 다시 읽는다** (`app/src/lib/gameSession.js`). 판 자체를 브로드캐스트에 실으면 순서가 보장되지 않아 오래된 판이 새 판을 덮을 수 있다. 받은 판을 되쏘지 않도록 `applyingRemote` 플래그로 한 번 걸러낸다 — 없으면 두 기기가 서로 밀어내며 무한히 돈다. 게임 결과 기록은 **방을 만든 쪽(p1)만** 남긴다(둘 다 남기면 포인트가 두 번 쌓인다).
