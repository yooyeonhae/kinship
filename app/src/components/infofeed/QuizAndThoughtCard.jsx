...
/**
 * 2-4. 오늘의 넌센스 퀴즈 & 생각 질문 (하브루타/사고력)
 * - 넌센스 퀴즈 ↔ 하브루타 질문 탭 전환
 * - 넌센스 정답 맞추기 및 정답 확인 토글
 * - 정답이 없는 열린 생각 질문과 미니 생각 적기
 */
function QuizAndThoughtCard({ onRemove }) {
  const [tab, setTab] = useState('quiz') // 'quiz' | 'havruta'
    ...
    <div className="relative bg-surface border-2 border-foreground rounded-2xl shadow-sticker p-4 flex flex-col justify-between shrink-0 w-[min(320px,76vw)] snap-start transition-all duration-200 min-h-[350px]">
      <div>
        ...
        {isQuiz ? (
          ...
          /* 탭 2: 하브루타 생각 질문 */
          <div>
            <div className="bg-pastel-sky/30 border border-primary/20 rounded-xl p-3 mb-2.5">
        ...
        >
          <span>
            {isQuiz
              ...
              : `다른 질문 보기 (${havrutaIndex + 1}/${HAVRUTA_QUESTIONS.length})`}
          </span>
