...
**v4 (이번 업데이트, `app/` React 화면 기준)** — 사용자 피드백으로 공용 헤더/하단 네비게이션 조정.

- **헤더에서 `Kinship` 워드마크 제거**: 모든 화면 공통 헤더 왼쪽 자리에는 이제 오늘 날짜(`예: 2026년 8월 18일 화요일`, `Intl.DateTimeFormat('ko-KR', {year, month, day, weekday})`)를 표시한다. 오른쪽 지구본/계정 아이콘은 유지.
- **헤더의 지구본/계정 아이콘 제거**: 이 앱에는 다국어도 로그인도 없어서 둘 다 눌러도 아무 일이 없는 `aria-hidden` 장식이었다. 헤더에는 이제 가족 이름만 남는다.
- **이모지 사용 범위 확대 (v4의 "하단 탭만 예외" 규칙 갱신)**: 사용자 요청으로 ⓐ 하단 5탭에 **색을 입혔고**(활성 탭은 탭별 색 배경 + 검은 테두리, 비활성은 흑백 처리 대신 살짝 흐리게만 — grayscale이 이모지 색을 통째로 죽였다), ⓑ **부모 할일 화면 상단 액션**을 Phosphor에서 이모지로 바꿨다(👕 지정복 · 📅 스케줄 · 👨‍👩‍👧 완료현황, 각각 색 배경 + 테두리). 뒤로 가기만 Phosphor `caret-left`를 유지한다 — 방향 화살표 이모지는 플랫폼마다 모양이 크게 달라 내비게이션 컨트롤로 쓰기 어렵다.
- **구성원 캐릭터(이모지 아바타)**: 이름 첫 글자 대신 구성원이 고른 캐릭터를 쓴다(`members.avatar`, migration_11). 글자를 못 읽는 아이도 자기 자리를 찾을 수 있고, 형제 이름의 첫 글자가 같을 때(서아·서연) 색 말고 다른 단서가 생긴다. 고르지 않았으면 이름 해시로 정해 늘 같은 캐릭터가 붙는다(`fallbackCharacter`). 적용: 온보딩 생성, 진입(로그인) 화면, 부모 PIN 화면, 부모 할일 카드의 담당자, 아지트 접속 표시·채팅. 캐릭터 변경은 **부모 할일 화면의 「가족 캐릭터」 카드**에서 한다(`members` 쓰기가 부모 전용이라 그 화면이 제자리다).
- **하단 네비게이션 아이콘 = 이모지 (예외)**: 원래 "이모지 금지, Phosphor로 통일" 규칙이 있었지만, 아이들이 보는 화면이라는 점을 고려해 **하단 탭 5개 아이콘만 예외적으로 이모지**를 쓴다 — 🏠 홈 · ✅ 할일 · 📰 정보 · 🎈 주말 · ⛺ 아지트. 그 외 화면 전체(카드 장식, 상태 아이콘 등)는 기존 규칙대로 Phosphor 아이콘 유지, 이모지 금지.
- 이 변경은 현재 `app/`(React) 쪽 `Header`/`BottomNav` 컴포넌트에만 적용됨. `screens/*.html`(정적 버전)의 헤더/`js/bottomnav.js`는 아직 이전 방식(Kinship 워드마크 + Phosphor 아이콘) 그대로 — 필요시 별도로 맞출 것.

**v3 (이전 업데이트)** — `kinship/*.png` 실제 목업과 다시 대조해서, 카드/버튼/헤더를 그 이미지에 더 가깝게 맞췄다. 하단 5탭 네비게이션(`bottomnav.js`)과 아이콘 기반 아바타(실사진 없음)는 그대로 유지하고, **레이아웃 구조가 아니라 카드 스타일·색·헤더 크롬만** 바꿨다.
